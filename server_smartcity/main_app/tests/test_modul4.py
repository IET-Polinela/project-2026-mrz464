...
class CRUDAndValidationTests(APITestCase):
    ...
    def test_FT_01_buat_laporan_dengan_data_lengkap(self):
        ...
        payload = {
            ...
            'description': 'Tiang listrik roboh menghalangi jalan utama.',
            'location': 'Jl. Raya Pasir Sakti',
        }
        response = self.client.post(url, payload, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(Report.objects.filter(title='Tiang Listrik Roboh').exists())
        report = Report.objects.get(title='Tiang Listrik Roboh')
    ...
    def test_FT_02_ditolak_jika_judul_kosong(self):
        ...
        PENJELASAN TEKNIS:
            ...
            tidak memiliki blank=True and null=True. Field `title` dengan
