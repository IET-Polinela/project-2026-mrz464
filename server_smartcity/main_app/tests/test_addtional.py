...
class SerializerAndModelCoverageTests(APITestCase):
    ...
    def test_report_serializer_no_request_context(self):
        ...
        """
        from dashboard_24782086.serializers import ReportSerializer
        report = Report.objects.create(
...
class MainAppMonolithicViewsCoverageTests(TestCase):
    ...
    def test_report_detail_api_valid(self):
        from dashboard_24782086.views import report_detail_api
    ...
    def test_report_search_unauthenticated(self):
        # Gunakan 'api_search' karena di urls.py namanya 'api_search', bukan 'report_search'
        response = self.client.get(reverse('dashboard:api_search') + '?q=Lampu')
    ...
    def test_report_search_admin(self):
        ...
        response = self.client.get(reverse('dashboard:api_search') + '?q=Monolitik')
        self.assertEqual(response.status_code, 200)
    ...
    def test_report_update_status_view_unauthenticated(self):
        response = self.client.post(reverse('report_status', kwargs={'pk': self.report.id}), {'status': 'VERIFIED'})
        self.assertEqual(response.status_code, 302)
